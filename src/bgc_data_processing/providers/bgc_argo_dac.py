...
loader = DataSource(
    provider_name="BGC_ARGO_DAC",
    data_format="netcdf",
    dirin=Path(PROVIDERS_CONFIG["BGC_ARGO_DAC"]["PATH"]),
    data_category=PROVIDERS_CONFIG["BGC_ARGO_DAC"]["CATEGORY"],
    excluded_files=PROVIDERS_CONFIG["BGC_ARGO_DAC"]["EXCLUDE"],
    ...
    variable_ensemble=SourceVariableSet(
        ...
        oxygen=VARS["oxygen"]
        .in_file_as(("DOXY_ADJUSTED", "DOXY_ADJUSTED_QC", ["1", "2", "5","8"]),
                    ("DOXY", "DOXY_QC", ["1", "2", "5","8"])) 
        .correct_with(units.convert_umol_by_kg_to_mmol_by_m3),
        phosphate=VARS["phosphate"].not_in_file(),
        nitrate=VARS["nitrate"].in_file_as(("NITRATE_ADJUSTED", "NITRATE_ADJUSTED_QC", ["1", "2", "5","8"]),
                                           ("NITRATE", "NITRATE_QC", ["1", "2", "5","8"])),  
        silicate=VARS["silicate"].not_in_file(),
        chlorophyll=VARS["chlorophyll"]
        .in_file_as(
           ("CHLA_ADJUSTED", "CHLA_ADJUSTED_QC", ["1", "2", "5","8"]),
           ("CHLA", "CHLA_QC", ["1", "2", "5","8"]), 
        )
        .remove_when_all_nan()
    ...
    bbp700=VARS["bbp700"].in_file_as(
        ("BBP700_ADJUSTED", "BBP700_ADJUSTED_QC", ["1", "2", "5","8"]),
        ("BBP700", "BBP700_QC", ["1", "2", "5","8"])),
